...
public class ShootTextProController : MonoBehaviour
{
    ...
    public Transform ShootTextCanvas
    {
        get
        {
            ...
            {
                shootTextCanvas = GameObject.Find("Hex Map Editor").transform;
            }
            if (shootTextCanvas == null)
            {
    ...
    private void Initialized()
    {
        ...
        //加法图片
        numberDic.Add(TextAnimationType.Normal + operatorPlusKeyPostfix, Resources.Load<GameObject>("Prefabs/" + TextAnimationType.Normal + operatorPlusKeyPostfix));
        numberDic.Add(TextAnimationType.Burst + operatorPlusKeyPostfix, Resources.Load<GameObject>("Prefabs/" + TextAnimationType.Burst.ToString() + operatorPlusKeyPostfix));
        //减法图片
        numberDic.Add(TextAnimationType.Normal + operatorMinusKeyPostfix, Resources.Load<GameObject>("Prefabs/" + TextAnimationType.Normal + operatorMinusKeyPostfix));
        numberDic.Add(TextAnimationType.Burst + operatorMinusKeyPostfix, Resources.Load<GameObject>("Prefabs/" + TextAnimationType.Burst + operatorMinusKeyPostfix));

        for (int i = 0; i < 10; i++)
        {
            numberDic.Add(TextAnimationType.Normal + numberPrefix + i, Resources.Load<GameObject>("Prefabs/" + TextAnimationType.Normal + numberPrefix + i));
        }
        for (int i = 0; i < 10; i++)
        {
            numberDic.Add(TextAnimationType.Burst + numberPrefix + i, Resources.Load<GameObject>("Prefabs/" + TextAnimationType.Burst + numberPrefix + i));
        }
    }
    ...
    private void Update()
    {
        ...
        foreach (var shootTextComponent in handleShootTextGroup)
        {
            var textCreatePosition = shootTextComponent.cacheTranform.GetComponent<Collider>().bounds.center + (((Vector3.up * shootTextComponent.cacheTranform.GetComponent<Collider>().bounds.size.y) * 0.5f));
            textCreatePosition.x += (float)shootTextComponent.initializedHorizontalPositionOffset;
            textCreatePosition.y += (float)shootTextComponent.initializedVerticalPositionOffset;

            var anchors = ShootTextCamera.WorldToViewportPoint(textCreatePosition);//飘字初始锚点位置
            var changeAnchoredPosition = new Vector2((float)(anchors.x + shootTextComponent.xMoveOffeset), (float)(anchors.y + shootTextComponent.yMoveOffeset));//飘字这一帧所在位置
            ...
            //处理近大远小
            var objectHigh = ModelInScreenHigh(shootTextComponent.cacheTranform);
            var scale = (objectHigh / 100) * shootTextScaleFactor;

            shootTextComponent.ChangeScale(scale);
            var xMoveOffset = horizontalMoveSpeed * deltaTime * objectHigh;
            var yMoveOffset = verticalMoveSpeed * deltaTime * objectHigh;
            ...
            if (shootTextComponent.isMove == true)//处理位置信息
            {
                switch (shootTextComponent.moveType)
                {
                    ...
                    {
                        shootTextComponent.yMoveOffeset += yMoveOffset;
                    }
                        break;
                    ...
                    {
                        shootTextComponent.xMoveOffeset -= xMoveOffset;
                    }
                        break;
                    ...
                    case TextMoveType.LeftParabola:
                    {
                        ...
                        shootTextComponent.xMoveOffeset -= xMoveOffset;
                        shootTextComponent.yMoveOffeset += yMoveOffset + parabola;
                    }
                        break;
    ...
    public void CreateShootText(string content, Transform targetTransform)
    {
        CreateShootText(content, this.textAnimationType, this.textMoveType, this.delayMoveTime, this.initializedVerticalPositionOffset, this.initializedHorizontalPositionOffset, targetTransform);
    }
    public void CreateShootText(string content, TextAnimationType textAnimationType, TextMoveType textMoveType, float delayMoveTime, float initializedVerticalPositionOffset, float initializedHorizontalPositionOffset, Transform tempTransform)
    {
        ...
        CreateShootText(shootTextInfo);
    }
    public void CreateShootText(ShootTextInfo shootTextInfo)
    {
    ...
    private void BuildNumber(ShootTextInfo shootTextInfo, Transform parent)
    {
        ...
        {
            var numberObj = Instantiate(numberDic[animationType + numberPrefix + tempNumber[i]]);
            numberObj.transform.SetParent(parent, false);
        }
        #endregion
    }

    private double ModelInScreenHigh(Transform payerTransform)
    {
        Vector3 playerCenter = payerTransform.GetComponent<Collider>().bounds.center;
        Vector3 halfYVector3 = (Vector3.up * payerTransform.GetComponent<Collider>().bounds.size.y) * 0.5f;
